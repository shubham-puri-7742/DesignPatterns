...
# Variant using an accept method as in statically-typed languages
...
class ExprPrinter:
    ...
    def visit(self, addExpr):
        self.buffer.append('(')
        # double dispatch because this calls accept, which in turn invokes the appropriate visit function
        # if the accept method is not implemented, this will fail (cf. the variant without accept)
        addExpr.left.accept(self)
        self.buffer.append(' + ')
        addExpr.right.accept(self)
        self.buffer.append(')')

    @visitor(MultExpr)
    def visit(self, multExpr):
        self.buffer.append('(')
        multExpr.left.accept(self)
        self.buffer.append(' × ')
        multExpr.right.accept(self)
        self.buffer.append(')')
